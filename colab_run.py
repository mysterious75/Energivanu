"""
ENERGIVANU — Colab Auto-Train (single file upload)
Upload this file to Google Colab, then run it.
Auto-clones repo, generates data, trains model.
"""

import os, sys, subprocess, pickle, numpy as np, time, shutil, json, warnings, gc
...
# Optimization
USE_AMP = True
USE_DP = False    # Colab has 1 GPU — DataParallel needs 2+
...
if not os.path.exists(REPO):
    print("Cloning repo...")
    subprocess.run(["git", "clone", "https://github.com/mysterious75/Energivanu.git", REPO],
...
print(f"{'='*70}")
print(f"  ENERGIVANU — Colab Training")
...
print(f"  Best SigAcc: {max(history['vs'])*100:.1f}% (target: >90%)")
